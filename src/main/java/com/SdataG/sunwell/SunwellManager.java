...
public final class SunwellManager {
    ...
    public void onSourceChanged(BlockPos pos, BlockState state, boolean isSource) {
        ...
        if (isSource) {
            ...
            this.sourceProfileByPos.put(packed, SunwellProfile.fromState(state));
        } else {
            ...
            for (int y = 0; y < 16; y++) {
                for (int z = 0; z < 16; z++) {
                    for (int x = 0; x < 16; x++) {
                        ...
                        if (blockState.is(Sunwell.SUNWELL_SOURCE)) {
                            if (found == null) {
                                found = new LongOpenHashSet();
                            }
                            long packed = BlockPos.asLong(baseX + x, baseY + y, baseZ + z);
                            found.add(packed);
                            this.sourceProfileByPos.put(packed, SunwellProfile.fromState(blockState));
                        }
                    }
                }
            }
