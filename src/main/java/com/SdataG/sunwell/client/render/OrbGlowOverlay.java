...
public final class OrbGlowOverlay {
    ...
    private static void drawDisc(VertexConsumer c, Matrix4f m, float radius, float cr, float cg, float cb) {
        for (int i = 0; i < DISC_SEGMENTS; i++) {
            ...
            c.addVertex(m, 0.0F, 0.0F, 0.0F).setColor(cr, cg, cb, 1.0F);
            c.addVertex(m, x0, y0, 0.0F).setColor(0.0F, 0.0F, 0.0F, 1.0F);
            c.addVertex(m, x1, y1, 0.0F).setColor(0.0F, 0.0F, 0.0F, 1.0F);
            c.addVertex(m, 0.0F, 0.0F, 0.0F).setColor(cr, cg, cb, 1.0F);
        }
    }
    ...
    private static void drawRays(VertexConsumer c, Matrix4f m, float br, float bg, float bb,
        ...
        for (int i = 0; i < RAY_COUNT; i++) {
            ...
            float tipX = dx * len;
            float tipY = dy * len;
            c.addVertex(m, px, py, 0.0F).setColor(br, bg, bb, 1.0F);
            c.addVertex(m, tipX + px * 0.25F, tipY + py * 0.25F, 0.0F).setColor(0.0F, 0.0F, 0.0F, 1.0F);
            c.addVertex(m, tipX - px * 0.25F, tipY - py * 0.25F, 0.0F).setColor(0.0F, 0.0F, 0.0F, 1.0F);
            c.addVertex(m, -px, -py, 0.0F).setColor(br, bg, bb, 1.0F);
        }
    }
    ...
    private static void beamVtx(VertexConsumer c, Matrix4f m, Vector3f p, Vector3f side, float k,
        ...
        c.addVertex(m, p.x + side.x * k, p.y + side.y * k, p.z + side.z * k).setColor(cr, cg, cb, 1.0F);
    }
}
