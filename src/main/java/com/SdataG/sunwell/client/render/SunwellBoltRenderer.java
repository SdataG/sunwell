...
public final class SunwellBoltRenderer {
    ...
    private static void vtx(VertexConsumer buffer, Matrix4f m, Vector3f p, Vector3f side, float k,
        ...
        buffer.vertex(m, p.x + side.x * k, p.y + side.y * k, p.z + side.z * k).color(cr, cg, cb, 1.0F).endVertex();
    }
    ...
    private static void drawGlow(VertexConsumer buffer, Matrix4f matrix, Vector3f camera, Vector3f pos,
        ...
        for (int i = 1; i <= seg; i++) {
            ...
            buffer.vertex(matrix, pos.x, pos.y, pos.z).color(cr, cg, cb, 1.0F).endVertex();
            buffer.vertex(matrix, prev.x, prev.y, prev.z).color(0.0F, 0.0F, 0.0F, 1.0F).endVertex();
            buffer.vertex(matrix, cur.x, cur.y, cur.z).color(0.0F, 0.0F, 0.0F, 1.0F).endVertex();
            buffer.vertex(matrix, pos.x, pos.y, pos.z).color(cr, cg, cb, 1.0F).endVertex();
            prev = cur;
        }
    }
